...
const LOCALIZATION = {
    en: {
        ...
        hero_title: 'Fox Media - Your Creative Partner',
        ...
        section_why_choose: 'Why Choose Fox Media?',
        section_claim_canva: 'Claim Canva Pro for Free!',
        section_claim_subtitle: 'We value your opinion. Share your feedback about Fox Media and get instant access to 1 Year of Canva Pro for free! 🎁',
        label_email: 'Email Address *',
        label_phone: 'Phone Number *',
        label_rating: 'Rate Your Experience *',
        label_comments: 'Your Feedback (Optional)',
        placeholder_comments: 'Tell us what you like about Fox Media or how we can improve...',
        ...
        footer_rights: '© 2024 Fox Media. All rights reserved.',
        footer_disclaimer: 'Fox Media is an authorized reseller of Adobe Creative Cloud licenses.',
    ...
    ta: {
        ...
        hero_title: 'Fox Media - உங்கள் Creative Partner',
        ...
        section_why_choose: 'ஏன் Fox Media-ஐ தேர்வு செய்ய வேண்டும்?',
        ...
        placeholder_comments: 'Fox Media பற்றி உங்கள் கருத்துகளை இங்கே பதிவிடவும்...',
        ...
        footer_rights: '© 2024 Fox Media. All rights reserved.',
        footer_disclaimer: 'Fox Media என்பது Adobe Creative Cloud Licenses-இன் Authorized Reseller ஆகும்.',
...
const CONTACT_CONFIG = {
    WHATSAPP_NUMBER: '919865488886',
    INSTAGRAM_URL: 'https://www.instagram.com/future.system007?utm_source=ig_web_button_share_sheet&igsh=ZDNlZDc0MzIxNw==',
    FACEBOOK_URL: 'https://www.facebook.com/profile.php?id=61580502636647',
...
async function contactViaWhatsApp(planType) {
    ...
    const message = `Hi Fox Media! I'm interested in the ${planType} plan (${priceText}). Can you provide more details and process my order?`;
...
function openWhatsApp(message = '') {
    const defaultMsg = "Hi! I'm interested in Adobe Creative Cloud plans from Fox Media. Can you help me?";
...
function claimInstaDiscount() {
    ...
    if (pendingDiscountPlan) {
        setTimeout(async () => {
            ...
            const priceText = plan ? formatPrice(plan.price) : '';
            const message = `Hi Fox Media! I followed your Instagram and want to claim the special discount price of ${priceText} for the ${pendingDiscountPlan} plan. How can I proceed with the payment?`;
...
async function buyNow(planType) {
    ...
    const plan = findPlan(planType);
    const priceText = plan ? formatPrice(plan.price) : '';
    const message = `Hi Fox Media! I want to buy the ${planType} plan (${priceText}). How can I proceed with the payment?`;
...
function startFlashTimer() {
    let endTime = localStorage.getItem('foxmedia_flash_end');
    if (!endTime || Date.now() > parseInt(endTime)) {
        endTime = Date.now() + (2 * 60 * 60 * 1000) + (45 * 60 * 1000) + (30 * 1000); // 2h 45m 30s
        localStorage.setItem('foxmedia_flash_end', endTime);
    }
    ...
    setInterval(() => {
        let diff = parseInt(localStorage.getItem('foxmedia_flash_end')) - Date.now();
        if (diff <= 0) {
            const newEndTime = Date.now() + (2 * 60 * 60 * 1000) + (45 * 60 * 1000) + (30 * 1000);
            localStorage.setItem('foxmedia_flash_end', newEndTime);
...
const CANVA_REWARD_LINK = 'https://www.canva.com/brand/join?token=foxmedia-pro-1year-free-reward'; // Replace with your actual Canva team invite link
...
function setupFeedbackForm() {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        if (!success) {
            ...
            try {
                const submissions = JSON.parse(localStorage.getItem('foxmedia_feedback') || '[]');
                submissions.push({ ...feedbackData, created_at: new Date().toISOString() });
                localStorage.setItem('foxmedia_feedback', JSON.stringify(submissions));
            } catch (e) {
